...
![image-20260111193731197](Note_img/image-20260111193731197.png)

场景：笔记本连接到网络，请求 `www.google.com` 页面

1. 连接网络的笔记本需要获取自己的 IP 地址、默认网关、DNS 服务器的地址。使用 DHCP 协议、

   DHCP 请求报文从上至下被封装为 UDP 报文、IP 报文、以太网帧

   以太网帧在 LAN 上广播，被 DHCP 服务器收到

   DHCP 服务器构造提供报文，给笔记本提供 IP 地址

   笔记本选择一个 IP 地址，然后广播 DHCP 请求报文，被 DHCP 服务器收到

   DHCP 服务器构造确认报文，笔记本收到后便获得了 IP 地址、默认网关、DNS 服务器的地址

2. DNS 请求被依次封装到 UDP、IP、以太网帧。为了发送给 DNS 服务器，先要获得默认网关的 MAC 地址，使用 ARP 协议

   发送 ARP 请求广播，被默认网关接收到，返回单播帧到笔记本。笔记本获得了默认网关的 MAC 地址

3. 将封装好 DNS 的以太网帧发送给默认网关，默认网关通过路由协议转发给 DNS 服务器，DNS 服务器将 `www.google.com` 的 IP 地址再转发给笔记本

4. 笔记本通过 Google 的 IP 地址和 Google 服务器建立 TCP 连接（三次握手）

   HTTP 请求通过 TCP 发送报文到 Google 服务器

   Google 服务器返回 http 响应，路由到笔记本